...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < 1; ++i) {
    ...
    if (myRank == 0) {
      ...
      for (int j = 1; j < 5; ++j) {
	std::cout << "==== 32 BEFORE ALLREDUCE MPI RANK " << myRank << ", INDEX " << j << ": " << static_cast<float>(*(tmpCpu32 + j)) << std::endl;
      }
    }
    /*
      if (myRank == 1) {
    ...
    */
    ...
    if (myRank == 0) {
      ...
      for (int j = 1; j < 5; ++j) {
	std::cout << "==== 32 AFTER ALLREDUCE MPI RANK " << myRank << ", INDEX " << j << ": " << static_cast<float>(*(tmpCpu32 + j)) << std::endl;
      }
    }
    /*
    if (myRank == 1) {
